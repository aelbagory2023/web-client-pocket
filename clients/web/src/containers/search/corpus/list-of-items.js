...
import { loadMoreSearchItems } from './search.corpus.state'
import { loadPreviousSearchItems } from './search.corpus.state'
...
export const ListOfCorpusItems = () => {
  const dispatch = useDispatch()
  const viewport = useViewport()

  const pageSearchIds = useSelector((state) => state.pageSearchCorpusIds)
  const type = 'grid'
