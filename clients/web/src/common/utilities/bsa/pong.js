...
import { ADS_SIGNING_KEY, ADS_ZONE_MAPPING } from 'common/constants'
const coder = new Coder(ADS_SIGNING_KEY)
...
export function createPong2GetHandler() {
  return async (body, countryCode, userAgent) => {
    ...
    const placements = pongs
      .filter((p) => p in ADS_ZONE_MAPPING)
      .map((p) => {
        return { name: p, zoneKey: [ADS_ZONE_MAPPING[p]] }
      })
    ...
    const decisionRes = (await Promise.allSettled(requests))
      .filter((p) => {
        if (p.status === 'rejected') {
          console.error(`rejected ad request: ${p.reason}`)
        }
        return p.status === 'fulfilled'
      })
      .map((p) => p.value)

    const decisions = Object.fromEntries(decisionRes.map(({ name, p }) => [name, p]))

    if (pongs.every((p) => decisions[p] === null)) {
      const status = 'geo_unsupported'
